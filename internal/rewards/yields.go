...
func (r *Rewards) calculateCurrentYield(current_total_dly_supply *big.Int) *big.Int {
	// Current yield = (max supply - current total supply) / current total supply
	current_yield := big.NewInt(0).Sub(r.config.Chain.Hardforks.AspenHf.MaxSupply, current_total_dly_supply)
	current_yield.Mul(current_yield, YieldFractionDecimalPrecision)
	current_yield.Div(current_yield, current_total_dly_supply)
